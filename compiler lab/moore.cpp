...
void moore(char *str, int **arr, int initial, int states, int num) {
	...
	for (int i = 0; str[i] != '\0'; ++i) {
		...
		int n = arr[current][curr_input];

		current = arr[current][curr_input];
		if(n != -1) 
			std::cout << "  " <<arr[current][num - 1];
		else {
		 std::cout << " error ";
		 break;
		}
...
int main(){
	...
	while(true){
		...
		moore(str, arr, initial, states, num);
	}
}	